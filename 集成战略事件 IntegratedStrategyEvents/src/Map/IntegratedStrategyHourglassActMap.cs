...
/// <summary>
/// 沙漏形整幕地图生成器：7 列多路径，在距顶部固定距离处收束到一个全路径必经的
/// 中央固定节点，再向上散开至 BOSS。花瓣（特殊精英）与先知长角（秘境）两张图
/// 共用本算法，仅固定节点的类型与 RNG 流名不同。
/// </summary>
internal abstract class IntegratedStrategyHourglassActMap : ActMap
{
	...
	private readonly int _fixedNodeRow;
	private readonly MapPointType _fixedNodeType;
	...
	public MapCoord FixedNodeCoord { get; private set; }

	protected IntegratedStrategyHourglassActMap(IRunState runState, string rngName, MapPointType fixedNodeType)
	{
		ActModel act = runState.Act;
		bool isMultiplayer = runState.Players.Count > 1;
		_mapLength = act.GetNumberOfRooms(isMultiplayer) + 1;
		_rng = new Rng(runState.Rng.Seed, rngName);
		_fixedNodeType = fixedNodeType;
		_pointTypeCounts = act.GetMapPointTypes(_rng);
		Grid = new MapPoint[MapWidth, _mapLength];
		_fixedNodeRow = GetRowCount() - FixedNodeDistanceFromTop;
	...
	protected static bool TryGetFixedNodeCoord(ActMap map, MapPointType expectedType, out MapCoord coord)
	{
		coord = default;
		...
		MapCoord fixedNodeCoord = new()
		{
			col = col,
			row = row
		};
		MapPoint? point = map.HasPoint(fixedNodeCoord) ? map.GetPoint(fixedNodeCoord) : null;
		if (point == null || point.PointType != expectedType)
		{
			return false;
		}
	...
	{
		if (_fixedNodeRow <= 0 || _fixedNodeRow >= GetRowCount())
		{
			throw new InvalidOperationException($"Fixed node row is out of bounds for {GetType().Name}.");
		}
		...
		MapPoint fixedNode = GetOrCreatePoint(CenterColumn, _fixedNodeRow);
		fixedNode.PointType = _fixedNodeType;
		fixedNode.CanBeModified = false;
		FixedNodeCoord = fixedNode.coord;
		foreach (MapPoint point in GetPointsInRow(_fixedNodeRow).ToList())
		{
			if (point != fixedNode)
			{
				RedirectToFixedNode(point, fixedNode);
			}
		}
	...
	private MapCoord GenerateNextCoord(MapPoint current)
	{
		...
		int distanceToFixedNode = _fixedNodeRow - current.coord.row;
		if (distanceToFixedNode > 3)
		{
			directions.StableShuffle(_rng);
		}
		else if (distanceToFixedNode > 0)
		{
	...
	{
		int center = GetColumnCount() / 2;
		int distanceToFixedNode = Math.Abs(row - _fixedNodeRow);
		int distanceToTop = _mapLength - 1 - row;
		int upperSpread = Math.Min(center, Math.Max(0, distanceToTop) + 1);
		int spread = Math.Min(center, Math.Min(distanceToFixedNode, upperSpread));
